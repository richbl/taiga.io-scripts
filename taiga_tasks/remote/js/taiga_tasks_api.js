...
function getAuthToken(taigaParams) {
  ...
  $.ajax({
    ...
    success: function(json) {
      ...
      getProjectIDfromName(taigaParams);
    },
 ...
 */
function getProjectIDfromName(taigaParams) {
  $.ajax({
    method: "GET",
    url: taigaParams.website + '/api/v1/projects',
    beforeSend: function(xhr) {
      xhr.setRequestHeader('Authorization', 'Bearer ' + taigaParams.authToken);
    },
    success: function(json) {
      showSuccessAlert("Project list retrieval succeeded.");

      var result = $.grep(json, function(element, index) {
        return element.name == taigaParams.projectName;
      });

      if (result[0] === undefined) {
        showErrorAlert("Unable to locate project name: " + taigaParams.projectName);
      } else {
        taigaParams.projectID = result[0].id;
...
/**
 * ----------------------------------------------------------------------------------
 * Get taiga project ID (DEPRECATED)
 */
...
/**
 * ----------------------------------------------------------------------------------
 * Get taiga project name (DEPRECATED)
 */
